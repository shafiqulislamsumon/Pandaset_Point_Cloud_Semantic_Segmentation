...
def lovasz_softmax_flat(probas, labels, classes='present'):
    ...
    if probas.numel() == 0:
        # only void pixels, the gradients should be 0
        #return probas * 0.
        return 0.  # Caution : Error handle
    
    #print("##### probas shape : ", probas.size())
    if len(list(probas.size())) == 1:
        '''
            Caution : Error handle
            probas shape : [43] that is 1D but it should be [80222, 43] that means 2D. So return 0 loss
        '''
        return 0.
