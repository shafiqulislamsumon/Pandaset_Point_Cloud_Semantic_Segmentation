...
from dataloader.pc_dataset import get_Pandaset_label_name
...
class IoU(Metric):
    def __init__(self, dataset_config, dist_sync_on_step=False, compute_on_step=True):
        ...
        self.hist_list = []
        self.best_miou = 0
        self.Pandaset_label_name = get_Pandaset_label_name(dataset_config["label_mapping"])
        self.unique_label = np.asarray(sorted(list(self.Pandaset_label_name.keys())))[1:] - 1
        self.unique_label_str = [self.Pandaset_label_name[x] for x in self.unique_label + 1]
