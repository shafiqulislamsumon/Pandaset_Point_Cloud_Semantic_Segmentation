...
def parse_config():
    ...
    parser.add_argument('--config_path', default='./config/panadaset_config.yaml')
    # training
    parser.add_argument('--log_dir', type=str, default='pandaset', help='log location')
    ...
    parser.add_argument('--checkpoint', type=str, default=None, help='checkpoint/pretrained_model.ckpt')
    # debug
...
def build_loader(config):
    ...
    test_config = config['dataset_params']['test_data_loader']
    ...
    if not config['test']:
        ...
        test_pt_dataset = pc_dataset(config, data_path=test_config['data_path'], imageset='test', num_vote=test_config["batch_size"])
        test_dataset_loader = torch.utils.data.DataLoader(
            dataset=dataset_type(test_pt_dataset, config, test_config, num_vote=test_config["batch_size"]),
...
if __name__ == '__main__':
    ...
    model_file = importlib.import_module('model.' + configs['model_params']['model_architecture'])
