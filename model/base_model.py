...
class LightningBaseModel(pl.LightningModule):
    ...
    def test_step(self, data_dict, batch_idx):
        ...
        self.val_acc(prediction, raw_labels)
        self.log('test/acc', self.val_acc, on_epoch=True)
        self.val_iou(
            prediction.cpu().detach().numpy(),
            raw_labels.cpu().detach().numpy(),
            )
    ...
    def validation_epoch_end(self, outputs):
        ...
        with open('./report/val_report.txt', 'a+') as f:
    ...
    def test_epoch_end(self, outputs):
        ...
        self.log('test/best_miou', best_miou, on_epoch=True)
        str_print += 'Test per class iou: '

        for class_name, class_iou in zip(self.val_iou.unique_label_str, iou):
            str_print += '\n%s : %.2f%%' % (class_name, class_iou * 100)

        str_print += '\nCurrent test miou is %.3f while the best test miou is %.3f' % (mIoU * 100, best_miou * 100)
        self.print(str_print)

        os.makedirs('report', exist_ok=True)
        with open('./report/test_report.txt', 'a+') as f:
            f.write("####### Test Report : " + str(self.current_epoch) + '\n')
            f.write(str_print)
            f.write('\n\n')
